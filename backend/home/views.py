...
def getOfferDetails(request, offer_id):
    try:
        ...
        response_data = {
            "image": (
                request.build_absolute_uri(offer.image.url) if offer.image else None
            ),
...
def getModels(request):
    try:
        ...
        for make in makes:
            ...
            makeModel[make.makeName].append(
                {"modelName": make.modelName, "model_id": make.model_id}
            )
        for makeName, modelList in makeModel.items():
            response_array.append({"makeName": makeName, "models": modelList})
        if len(response_array) == 0:
...
def getCompany(request, dealership_id):
    try:
        ...
        response_data = {
            "image": (
                request.build_absolute_uri(company.image.url) if company.image else None
            ),
...
def getOffersForCompany(request, dealership_id):
    try:
        ...
        if len(offer_array) == 0:
            return Response({"error": "Offers not found"}, status=404)
        response_data = {"offers": offer_array, "last": last}
        return JsonResponse(response_data, status=200)
    except Offer.DoesNotExist:
        return Response({"error": "Offers not found"}, status=404)
...
def getShowcasedCompanies(request):
    try:
            ...
            company_array.append(
                {
                    "companyName": company.user.first_name,
                    "companyLogo": (
                        request.build_absolute_uri(company.image.url)
                        if company.image
                        else None
                    ),
                    "dealership_id": company.dealership_id,
        ...
        response_data = {"companies": company_array}
        return JsonResponse(response_data, status=200)
    except Dealership.DoesNotExist:
        return Response({"error": "Companies not found"}, status=404)
        ...
        offers = (
            Offer.objects.annotate(
                value=ExpressionWrapper(
                    F("rating") / F("price"), output_field=DecimalField()
                )
            )
            .order_by("-value")
            .all()
...
def getCities(request):
    try:
        ...
        for country, cityList in countryCity.items():
            response_array.append({"countryName": country, "cities": cityList})
        if len(response_array) == 0:
...
def getFilteredOffers(request):
    try:
        ...
        if (
            ...
            or pick_up_date == None
            or pick_up_time == None
            or drop_off_date == None
            or drop_off_time == None
        ):
        ...
        working_hours = WorkingHours.objects.filter(
            location__in=pick_up_locations
        ...
        if pick_up_locations.count() == 0:
            return Response(
                {"error": "No locations are open at the specified pickup time"},
                status=404,
            )
        pick_up_dealers_list = pick_up_locations.distinct("dealership_id").values_list(
            "dealership_id", flat=True
        )
        pick_up_dealers = Dealership.objects.filter(
            dealership_id__in=pick_up_dealers_list
        )
        ...
        if drop_off_location != None:
            ...
            drop_off_locations = Location.objects.filter(
                cityName=drop_off_cityName, countryName=drop_off_countryName
            )
            # remove locations that are closed at drop off time
            working_hours = WorkingHours.objects.filter(
                location__in=drop_off_locations
            ...
            if drop_off_locations.count() == 0:
                return Response(
                    {"error": "No locations are open at the specified dropoff time"},
                    status=404,
                )
            drop_off_dealers_list = drop_off_locations.distinct(
                "dealership_id"
            ).values_list("dealership_id", flat=True)
            pick_up_dealers = pick_up_dealers.filter(
                dealership_id__in=drop_off_dealers_list
            )
            if pick_up_dealers.count() == 0:
                return Response(
                    {
                        "error": "Pick up and drop off locations are not owned by the same dealer"
                    },
                    status=404,
                )
        ...
        vehicles = vehicles.filter(location__in=pick_up_locations).filter(
            dealer__in=pick_up_dealers
        )
        pick_up_datetime = datetime.combine(
            pick_up_date, datetime.min.time()
        ) + timedelta(hours=pick_up_time.hour)
        drop_off_datetime = datetime.combine(
            drop_off_date, datetime.min.time()
        ) + timedelta(hours=drop_off_time.hour)
        active_rents = Rent.objects.filter(
            Q(
                dateTimeRented__lte=pick_up_datetime,
                dateTimeReturned__gte=pick_up_datetime,
            )
            | Q(
                dateTimeRented__lte=drop_off_datetime,
                dateTimeReturned__gte=drop_off_datetime,
            )
        ).filter(vehicle__in=vehicles)
        rented_vehicles = active_rents.distinct("vehicle_id").values_list(
        ...
        for offer in offers:
            if (
                available_vehicles.filter(model=offer.model)
                .filter(dealer=offer.dealer)
                .exists()
            ):
                offers_list.append(
                    {
        ...
        if list_len == 0:
            return Response(
                {"error": "No vehicles available for this time period!"}, status=404
            )
        offset = (page - 1) * limit
        last = False
        if list_len < offset + limit + 1:
            last = True
        response_data = {"offers": offers_list[offset : offset + limit], "last": last}
        return JsonResponse(response_data, status=200)
    except Offer.DoesNotExist:
        return Response({"error": "Offers not found"}, status=404)
...
def getReviews(request, offer_id):
    try:
        ...
        response_data = {"reviews": review_array, "last": last}
        return JsonResponse(response_data, status=200)
    except Offer.DoesNotExist:
        return Response({"error": "Offer not found"}, status=404)
    ...
    try:
        ...
        if pickUpDate != None:
            ...
            if pickUpTime < 0 or pickUpTime > 23:
                return Response({"error": "Invalid time format"}, status=404)
            pickUpDateTime = datetime.combine(
                pickUpDate, datetime.min.time()
            ) + timedelta(hours=pickUpTime)
        ...
        if pickUpDateTime != None:
            active_rents = Rent.objects.filter(vehicle__in=vehicles).filter(
                dateTimeRented__lte=pickUpDateTime, dateTimeReturned__gte=pickUpDateTime
            )
        ...
        response_data = {"locations": locations_array}
        return JsonResponse(response_data, status=200)
    except Offer.DoesNotExist:
        return Response({"error": "Offer not found"}, status=404)
...
def getUnavailablePickupTimes(request, offer_id):
    ...
    vehicles = (
        Vehicle.objects.filter(location=pickUpLocation)
        .filter(dealer=offer.dealer)
...
def getLastAvailableDropOffTime(request, offer_id):
    ...
    for vehicle in vehicles:
        rentals = Rent.objects.filter(vehicle=vehicle).filter(
            dateTimeRented__gt=pickUpDateTime
        )
        if rentals.count() == 0:
            lastReturnDateTime = None
        ...
        if vehicleReturnDateTime == False:
            return Response(
                {"error": "No working hours for drop off location"}, status=404
            )
...
def getFirstAvailableWorkingTime(returnDateTime, dropOffLocation):
    ...
    if wrongDay:
        ...
        if returnDateTime.hour > workingHours[0].endTime.hour:
            ...
            while workingHours.count() == 0:
                returnDateTime -= timedelta(days=1)
                workingHours = dropOffWorkingHours.filter(
                    dayOfTheWeek=returnDateTime.weekday()
                )
            returnDateTime = returnDateTime.replace(
                hour=workingHours[0].endTime.hour, minute=0, second=0
            )
